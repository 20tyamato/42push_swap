...
TRACE=./push_swap_tester/trace_basic
RED='\033[0;31m'
GREEN='\033[0;32m'
NOCOLOR='\033[0m'
...
do
	printf "test_case_$i\n\n"
	# input=$(cat $TRACE/identity_files/test_case_$i.txt)  # テストケースの内容を変数に読み込む
    # printf "Input: $input\n\n"
	# printf "($ROOT/push_swap "$(cat $TRACE/identity_files/test_case_$i.txt)" >stdout 2>stderr)\n"
...
do
	...
	then
		...
		./push_swap_tester/files/checker "$(cat $TRACE/size5/test_case_$i.txt)" < stdout 1> result 2>error_result
		echo "OK" > ok
		if [[ -s error_result ]];
		then
			cat error_result
