...
make -C ./push_swap_tester/files/
g++ -O3 ./push_swap_tester/files/main.cpp -o ./push_swap_tester/files/better_random_test_cases
for ((i=1;i<=$2;i++));
do
./push_swap_tester/files/better_random_test_cases $@ > ./push_swap_tester/trace_loop/test_case_$i.txt
done
...
do
printf "$i	"
($ROOT/push_swap $(cat ./push_swap_tester/trace_loop/test_case_$i.txt) > ./push_swap_tester/trace_loop/output_$i.txt) & pid=$!
...
if [[ "$TLEFLAG" = "0" ]];
then
leaks -atExit -- $ROOT/push_swap $(cat ./push_swap_tester/trace_loop/test_case_$i.txt) > a 2>c
...
count=$(cat ./push_swap_tester/trace_loop/output_$i.txt | wc -l)
printf "$count "
total=$((total + count))
if [ $count -gt $max ]
then
...
printf "${RED}"
cat ./push_swap_tester/trace_loop/output_$i.txt | ./push_swap_tester/files/checker $(cat ./push_swap_tester/trace_loop/test_case_$i.txt) > temp_result
printf "${NOCOLOR}"
...
rm ./push_swap_tester/files/better_random_test_cases
